...
GitHub Actions 按 UTC 日期缓存这五个 delegated 文件，并在下载前恢复最近的缓存。每次下载先写入临时文件，只有非空下载成功后才原子替换现有文件；单个 RIR 下载失败时使用已恢复的非空缓存。若对应文件既无法下载也没有缓存，生成仍会失败，避免发布缺少任一 RIR 的部分结果。
